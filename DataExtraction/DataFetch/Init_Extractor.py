...
def main(args):
    ae.set_agent('smeguy@163.com')
    download_only = True
    set_sleep = True
    extract_only = False
    download_and_extact = False
    init_project = True
    ...
    if len(args) > 0:
        for arg in args:
            if arg == '-e':
                ...
                print('Please insert the right command:\n-e only extract report from predownloaded data\n-d only download reports\n-s Turn on sleep to make download more stable\n-u up-to-date the data')
                sys.exit()
    ...
    if download_and_extact:
        ...
        for folder in sub_folders:
            ...
            if os.path.exists(t_f):
                ae.download_and_extract_archive_report(year = int(folder.split('q')[0]), index_file = t_f, active_cik = active_cik, sic_filter = sic_filter, conn = conn, update = False)
