...
def download_one_historical_stock(symbol):
    url = cons_url(symbol)
...
def insert_his_stock(symbol, time_series):
    handler = DBHandler()
...
def download_all_historical_stock():
    ...
    for i in range(0, 10):
        ...
        print("Start download and storing %s. %s..." % (key, symbol))
        try:
            time_series = download_one_historical_stock(symbol)
        except Exception as result:
            print(result)
            download_failures.append(symbol)
        else:
            print("%s. %s successfully downloaded." % (key, symbol))
            insert_his_stock(symbol, time_series)
            print("%s. %s successfully stored." % (key, symbol))
            print()
